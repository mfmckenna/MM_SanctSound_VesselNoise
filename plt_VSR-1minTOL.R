# plots the output of process-VesselDetections_TOL-1min VSR
...
inDir = "C:\\Users\\megan\\Documents\\combineFiles_VesselSpeedReduction"

# LOAD RESULTS by Site and combine by sites ####
load( paste0(inDir, "\\outputTOL_OC02_2023-06-05.Rda") )
outputTOL = TOLmin
load( paste0(inDir, "\\outputVD_OC02_2023-06-05.Rda") )
...
# VD occurrence plot 
# how is vessel noise distributed across time
...
# Seasonal Patterns ####
#percent time with vessel or not by yr + month
TOLc = as.data.frame( outputTOL %>% group_by(Yr, Label, mth) %>% tally() ) # TOTAL minutes per month + condition + site
TOLcT = as.data.frame( outputTOL %>% group_by(Yr, mth) %>% tally() )      # TOTAL minutes per month + site
ND = merge(TOLc, TOLcT, by = c("Yr", "mth"))
ND$perTime = ND$n.x/ ND$n.y 
# add slow down label to the data
ND = ND[ ND$Label == "A. Vessel Detection", ]
ND$slow = "No"
ND$slow [ ND$Yr > 2019 & ND$mth == 7] = "Slowdown"
# START HERE ####
ND

## SNR Vessel DETECTIONS ####
...
# mean and standard error max noise above closest non-vessel periods (for all vessel detection periods + AIS label)
VExcee_mu  = aggregate(outputVD3$SNRmax, by=list(outputVD3$Yr, outputVD3$Mth), mean, na.rm=T)
VExceed_se = aggregate(outputVD3$SNRmax, by=list(outputVD3$Yr, outputVD3$Mth), std.error, na.rm=T)
VD_n = as.data.frame( outputVD3 %>% group_by(Yr, Mth) %>% tally() ) 

Vexceed = merge( VExcee_mu, VExceed_se, by = c("Group.1","Group.2"), all.y = FALSE) 
colnames(Vexceed ) = c("Yr", "Mth","SNRmax_mean","SNRmax_se")
Vexceed = merge( Vexceed, VD_n, by = c("Yr", "Mth") )
colnames(Vexceed ) = c("Y", "mth","SNRmax_mean","SNRmax_se","VDs") 

# VD SEASONAL TRENDS ####
# with 1-min TOLs
# y-axis METRIC = month, x = Exceedance
# SHAPE METRIC = Yr, color = dominance, size = AIS
