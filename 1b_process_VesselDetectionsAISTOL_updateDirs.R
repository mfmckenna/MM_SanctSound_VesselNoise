...
# OUTPUT details ####
dDir = "F:\\SanctSound\\"
tDir = "F:\\SanctSound\\analysis\\"
outDir =  paste0(tDir,"analysis\\combineFiles_VesselManuscript\\TOL-daily\\")
DC = Sys.Date()
...
#dirVD     = (paste0(tDir,"analyis\\data2\\SanctSound_VesselDetection_DataProducts") )
...
dirAIS      = paste0(tDir,"AIS")
Fpattern = "_2018_10_to_2021_04.csv" # "_2018_10_to_2021_04.csv"
...
x = strsplit(basename( inFilesAIS) ,"_") 
...
inFileTOL = list.files(path=dDir, pattern = "_TOL_1h.csv", full.names = TRUE, recursive = TRUE) 
inFileTOL = inFileTOL[ !grepl("analysis", inFileTOL)]
inFileBB = list.files(path=dDir, pattern = "_BB_1h.csv", full.names = TRUE, recursive = TRUE) 
inFileBB = inFileBB[ !grepl("analysis", inFileBB)]
...
for (ss in 1:length(sitesVD)  ) { # ss = 12
  ...
  inFiles = inFileTOL[ grepl(deply, inFileTOL)]
  ...
  inFiles = inFileBB[ grepl(deply, inFileBB)]
  nFiles = length( infiles )
  ...
  ## PROCESS VESSEL DETECTION DATA-- daily metrics ####
  VESS = NULL
  ...
  if ( nrow(cData) != 0 ) {
    ...
    #works better for many comparative metrics-- over the months
    ...
    ### proportion of hours with vessels detected in entire month ####
    ...
    # DAILY differences from the hourse... so can plot variation on the x-axis
    totaHrsVessD = aggregate(hData$TotalVesselDetP, by=list(hData$Mth,  hData$Day), sum, na.rm=T)
    totalHrsD    = hData %>% count(Mth,Day)
    NVessMed   = aggregate(hrsNOVess[ixF], by=list(hrsNOVess$Mth), median, na.rm=T)
